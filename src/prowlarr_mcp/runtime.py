...
APP = "prowlarr"
ENV_URL = "PROWLARR_URL"
ENV_KEY = "PROWLARR_API_KEY"
DEFAULT_URL = "http://127.0.0.1:9696"
DEFAULT_PORT = 8444
...
mcp = FastMCP(
    ...
    instructions=(
        "Manage a Prowlarr instance: indexers and their definitions, indexer "
        "proxies, applications it syncs to, download clients, search across "
        "indexers, history, statistics and system settings. Every API "
        "operation is a tool, named verb-first: list_* reads a collection, "
        "get_*_by_id reads one record, create_*, update_* and delete_* change "
        "them. Use list_search to query every indexer at once, "
        "list_indexer to see what is configured, and list_indexer_schema for "
        "the definitions available to add. Before writing a resource, read it "
        "first and send the whole object back as body -- Prowlarr replaces "
        "the record rather than merging."
    ),
)
